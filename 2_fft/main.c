...
int main( void )
{
    ...
    while (1)
    {
        OLED_decimal(0,2,wave_freq,4,1,8);
        if(FFT_finish !=0)
        {
            ...
            arm_cfft_f32(&arm_cfft_sR_f32_len1024, (float32_t *) gDstBuffer_f32, 0, 1);
            ...
            FFT_finish = 0;
            if(gFFTmaxFreqIndex>512)gFFTmaxFreqIndex = 1024 - gFFTmaxFreqIndex;
            wave_freq = (float)gFFTmaxFreqIndex /1024.0*500;  // 采样频率1000khz
